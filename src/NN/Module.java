...
public class Module implements NeuralNetInterface{
    ...
    public void save(File argFile) {
        ...
        for(Object module: this.modules){
            if(module instanceof NeuralNetInterface){
                Map<Integer, Object> sub_state_dict = new HashMap<>();
                sub_state_dict.put(0, ((NeuralNetInterface) module).state_dict());
                state_dict.put(keyCounter, sub_state_dict);
                keyCounter += 1;
            }
        }
        try {
            // outputStream
            FileOutputStream outStream = new FileOutputStream(argFile.getAbsolutePath());
            ObjectOutputStream objectOutputStream = new ObjectOutputStream(outStream);
            objectOutputStream.writeObject(state_dict);
            outStream.close();
        } catch (IOException e) {
            System.out.println("[ERROR] An error occurred when saving state dict.");
        }
    }
    ...
    public void load(String argFileName) throws IOException {
        ...
        try {
            ...
            this.load_state_dict(map);
            objectInputStream.close();
        } catch (FileNotFoundException | ClassNotFoundException e) {
            System.out.println("[ERROR] Unable to load state dict from file " + argFileName);
        }
    }
